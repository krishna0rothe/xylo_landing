  ...
  const SpotlightCard: React.FC<{
    ...
    return (
      <div
        ...
        className={`relative rounded-3xl border border-neutral-800 bg-neutral-900 overflow-hidden p-8 transition-all duration-300 ease-in-out hover:scale-105 ${className} flex flex-col h-full`}
      >
        <div
  ...
  return (
    <ParallaxProvider>
      <section ref={sectionRef} className="py-20 bg-black text-white overflow-hidden">
        <div className="container mx-auto px-4 relative pt-8 sm:pt-16">
          <Parallax translateY={[-20, 20]}>
            <motion.h2
              initial={{ opacity: 0, y: -50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.2 }}
              className="text-3xl sm:text-4xl md:text-4xl lg:text-4xl font-bold text-center mb-8 sm:mb-12 animate-fade-in-up sticky top-0 z-20 py-0"
            >
              Choose Your Plan
            </motion.h2>
          </Parallax>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10 pt-8 sm:pt-12">
            {plans.map((plan, index) => (
              <Parallax key={index} translateY={[20, -20]} scale={[0.9, 1.1]}>
                <motion.div
